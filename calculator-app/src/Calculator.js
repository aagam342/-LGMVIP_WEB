    ...
    try {
      ...
      const infixToPostfix = (input) => {
        ...
        input.split(/([\+\-\*\/\^\(\)])/).forEach((token) => {
          ...
          if (!isNaN(trimmedToken)) {
            outputQueue.push(trimmedToken);
          } else if (operators.includes(trimmedToken)) {
            while (
              operatorStack.length > 0 &&
              getPrecedence(operatorStack[operatorStack.length - 1]) >=
                getPrecedence(trimmedToken)
            ) {
      ...
      const postfixTokens = postfixExpression.split(" ");
